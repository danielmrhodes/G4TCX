...
DetectionSystemSpice::DetectionSystemSpice(G4bool chk) : check(chk) 
{
	/////////////////////////////////////////////////////////////////////
...
void DetectionSystemSpice::PlaceDetectorMount(G4LogicalVolume* exp_hall_log)
{
	...
	fDetectorMountPhys = new G4PVPlacement(rotate, pos, fDetectorMountLog,
					       "detector_mount", exp_hall_log, false, 0, check);
...
G4int DetectionSystemSpice::BuildPlaceSiliconWafer(G4LogicalVolume* ExpHallLog) 
{
	...
	G4Material* matWorld = G4Material::GetMaterial("G4_Galactic");
	if(!matWorld) {
	...
	//Place physical
	new G4PVPlacement(rotate,G4ThreeVector(0,0,0), fInnerGuardRingLog, "SiLiInnerGuard",  siDetMotherLog, false, 0, check);
	new G4PVPlacement(rotate,G4ThreeVector(0,0,0), fOuterGuardRingLog, "SiLiOuterGuard",  siDetMotherLog, false, 0, check);
	...
	for(int r=0;r<fSiDetRadialSegments;r++){
		...
		for(int sec=0;sec<fSiDetPhiSegments;sec++){
			...
			new G4PVPlacement(rotsec[sec],G4ThreeVector(0,0,0), fSiSegmentLog, temp.str(),  siDetMotherLog, false, 0, check);
		}	
	}
	...
	new G4PVPlacement(rotate,move, siDetMotherLog, "SiLiDetActive",  ExpHallLog, false, 0, check);
	return 1;
}
...
void DetectionSystemSpice::BuildDetectorMount() {
	...
	G4NistManager* man = G4NistManager::Instance();
	G4Material* DetectorMountMaterial = man->FindOrBuildMaterial("G4_Al");
...
void DetectionSystemSpice::BuildAnnularClamps() {
	...
	G4NistManager* man = G4NistManager::Instance();
	G4Material* AnnularClampMaterial = new G4Material("AnnularClampMaterial",1.31*g/cm3,3);
	AnnularClampMaterial->AddMaterial(man->FindOrBuildMaterial("G4_C"),0.76);
	AnnularClampMaterial->AddMaterial(man->FindOrBuildMaterial("G4_H"),0.08);
	AnnularClampMaterial->AddMaterial(man->FindOrBuildMaterial("G4_O"),0.16);
