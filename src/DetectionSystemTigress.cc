...
// at the origin
///////////////////////////////////////////////////////////////////////
void DetectionSystemTigress::Build() {
...
G4int DetectionSystemTigress::PlaceDeadLayerSpecificCrystal(G4LogicalVolume* expHallLog,
	...
	for(i=0; i<4; i++){
		...
		moveGermanium[i] = G4ThreeVector(DetectionSystemTigress::TransX(x,y,z,theta,phi), DetectionSystemTigress::TransY(x,y,z,theta,phi), DetectionSystemTigress::TransZ(x,z,theta));

		G4int copy = detectorNumber*4 + i + 1;	
		fGermaniumAssemblyCry[i]->MakeImprint(expHallLog, moveGermanium[i],
						      rotateGermanium[i], copy, fSurfCheck);
		//fGermaniumAssemblyCry[i]->MakeImprint(expHallLog, moveGermanium[i],
		//rotateGermanium[i], fCopyNumber++, fSurfCheck);
...
void DetectionSystemTigress::PlaceCrystal(G4LogicalVolume* exp_hall_log, G4int detectorNumber) {
  ...
  for(i=0; i<1; i++) {
    ...
    //G4RotationMatrix tmp = (G4RotationMatrix)rotateGermanium[i]->inverse();
    //G4RotationMatrix* rotateGe = new G4RotationMatrix(tmp);
    ...
    //G4double angle = rotateGermanium[i]->delta();
    //G4ThreeVector axis = rotateGermanium[i]->axis();
    //moveDeadLayer.rotate(axis,angle);
    
    //new G4PVPlacement(rotateGermanium[i],moveDeadLayer+moveGermanium[i],fInnerDeadLayerLog,
    //		      "GeDeadLayer",exp_hall_log,0,0,fSurfCheck);
...
///////////////////////////////////////////////////////////////////////
void DetectionSystemTigress::ConstructComplexDetectorBlock() {
	G4Material* materialGe = G4Material::GetMaterial("G4_Ge");
	if(!materialGe) {
		G4cout<<" ----> Material G4_Ge not found, cannot build the detector shell! "<<G4endl;
		exit(1);
...
void DetectionSystemTigress::ConstructComplexDetectorBlockWithDetectorSpecificDeadLayer(G4int det,
	...
	G4VisAttributes* germaniumBlock1VisAtt = new G4VisAttributes(fTigressCrystalColours[cry]);
	germaniumBlock1VisAtt->SetVisibility(true);

	G4VisAttributes* deadLayerVisAtt = new G4VisAttributes(fTigressDeadLayerColours[cry]);
	deadLayerVisAtt->SetVisibility(true);
	...
	G4double startAngle = 0.0*M_PI;
	G4double finalAngle = 2.0*M_PI;
	G4double deadLayerRadius = fGermaniumHoleRadius + fTigressDeadLayers[det][cry];
	G4double deadLayerHalfLengthZ = (fGermaniumLength
			- fGermaniumHoleDistFromFace
			+ fTigressDeadLayers[det][cry])/2.0;
	...
	// dead layer cap
	deadLayerRadius = fGermaniumHoleRadius;
	G4double deadLayerCapHalfLengthZ = (fTigressDeadLayers[det][cry])/2.0;

	G4Tubs* deadLayerCap = new G4Tubs("deadLayerCap", 0.0,
			deadLayerRadius, deadLayerCapHalfLengthZ, startAngle,finalAngle);

	G4ThreeVector moveDeadLayerCap(fGermaniumShift, -(fGermaniumShift),
			-((fGermaniumHoleDistFromFace - fTigressDeadLayers[det][cry])/2.0 - deadLayerHalfLengthZ + fTigressDeadLayers[det][cry]/2.0));
...
void DetectionSystemTigress::ConstructNewSuppressorCasingDetectorSpecificDeadLayer(G4int det, G4int cry) {
	G4String strdet = G4UIcommand::ConvertToString(det);
	G4String strcry = G4UIcommand::ConvertToString(cry);
...
G4SubtractionSolid* DetectionSystemTigress::QuarterSpecificDeadLayerDetector(G4int det, G4int cry) {
