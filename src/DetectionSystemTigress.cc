...
///////////////////////////////////////////////////////////////////////
// The ::DetectionSystemTigress constructor instatiates all the
...
// ::~DetectionSystemTigress destructor deletes them from the stack
// when they go out of scope
///////////////////////////////////////////////////////////////////////

DetectionSystemTigress::~DetectionSystemTigress() {
...
}// end ::~DetectionSystemTigress

///////////////////////////////////////////////////////////////////////
// ConstructDetectionSystemTigress builds the DetectionSystemTigress
// at the origin
///////////////////////////////////////////////////////////////////////

/// These functions describe a rotation of coordinates x,y,z by theta around the y-axis and then by phi around the z-axis
G4double DetectionSystemTigress::TransX(G4double x, G4double y, G4double z, G4double theta, G4double phi) {
  return (x*cos(theta)+z*sin(theta))*cos(phi)-y*sin(phi);
}

G4double DetectionSystemTigress::TransY(G4double x, G4double y, G4double z, G4double theta, G4double phi) {
  return (x*cos(theta)+z*sin(theta))*sin(phi)+y*cos(phi);
}

G4double DetectionSystemTigress::TransZ(G4double x, G4double z, G4double theta) {
...
G4int DetectionSystemTigress::PlaceEverythingButCrystals(G4LogicalVolume* expHallLog, G4int detectorNumber) {
  ...
  G4ThreeVector move(DetectionSystemTigress::TransX(x,y,z,theta,phi), DetectionSystemTigress::TransY(x,y,z,theta,phi), DetectionSystemTigress::TransZ(x,z,theta));
  ...
  move = G4ThreeVector(DetectionSystemTigress::TransX(x,y,z,theta,phi), DetectionSystemTigress::TransY(x,y,z,theta,phi), DetectionSystemTigress::TransZ(x,z,theta));
  ...
  if(fIncludeBackSuppressors) {
    ...
    for(i=0; i<4; i++) {
      ...
      moveBackQuarterSuppressor[i] = G4ThreeVector(DetectionSystemTigress::TransX(x,y,z,theta,phi),DetectionSystemTigress::TransY(x,y,z,theta,phi),DetectionSystemTigress::TransZ(x,z,theta));
  ...
  if(fIncludeSideSuppressors) {
    ...
    for(i=0; i<4; i++) {
      ...
      moveInnerSuppressor[i*2] = G4ThreeVector(DetectionSystemTigress::TransX(x,y,z,theta,phi),DetectionSystemTigress::TransY(x,y,z,theta,phi),DetectionSystemTigress::TransZ(x,z,theta));
      ...
      moveInnerSuppressor[i*2+1] = G4ThreeVector(DetectionSystemTigress::TransX(x,y,z,theta,phi),DetectionSystemTigress::TransY(x,y,z,theta,phi),DetectionSystemTigress::TransZ(x,z,theta));
  ...
  if(!(fSuppressorPositionSelector) && fIncludeExtensionSuppressors)
      ...
      for(i=0; i<4; i++)
	{
	  ...
	  moveInnerExtension[2*i] = G4ThreeVector(DetectionSystemTigress::TransX(x,y,z,theta,phi),DetectionSystemTigress::TransY(x,y,z,theta,phi),DetectionSystemTigress::TransZ(x,z,theta));
	  ...
	  moveInnerExtension[2*i+1] = G4ThreeVector(DetectionSystemTigress::TransX(x,y,z,theta,phi),DetectionSystemTigress::TransY(x,y,z,theta,phi),DetectionSystemTigress::TransZ(x,z,theta));
...
void DetectionSystemTigress::PlaceSegmentedCrystal(G4LogicalVolume* exp_hall_log, G4int detectorNumber) {
  ...
  // positioning
  G4double distFromOriginDet = fAirBoxBackLengthDet/2.0 +fAirBoxFrontLengthDet
    + fNewRhombiRadiusDet;

  G4double x0 = (fGermaniumWidth + fGermaniumSeparation)/2.0;
  G4double y0 = (fGermaniumWidth + fGermaniumSeparation)/2.0;
  G4double z0 = fGermaniumLength/2.0 + fCanFaceThickness/2.0 + fGermaniumDistFromCanFace + fShift
    + fAppliedBackShift + distFromOriginDet;
  ...
  G4double frontLayer = 0.4*mm;
  G4double backLayer = 0.05*mm;
  G4double innerLayer = 2.50*mm;
  G4double outterLayer = 0.20*mm;
  G4double middleLayer = 0.20*mm;
  G4bool full = false;
  std::vector<G4IntersectionSolid*> segments = SegmentedQuarterDetector(frontLayer,backLayer,innerLayer,outterLayer,middleLayer,full);
  
  G4LogicalVolume* geDeadL = NULL;
  if(!full) {
    G4SubtractionSolid* geFull = FullQuarterDetector();
    G4SubtractionSolid* geSmall = QuarterDetector(frontLayer,backLayer,innerLayer,outterLayer);
    G4SubtractionSolid* geDeadS = new G4SubtractionSolid("geDeadLayersS",geFull,geSmall,NULL,G4ThreeVector());
    geDeadL = new G4LogicalVolume(geDeadS,materialGe,"geDeadLayersL");
    ...
    geDeadL->SetVisAttributes(deadLayerVisAtt);
  }
  ...
  for(G4int i=start; i<stop; i++) {
    ...
    rotateGermanium[i]->invert();
    
    G4double x = -x0*pow(-1, floor((i+1)/2));
    G4double y = -y0*pow(-1, floor((i+2)/2));
    G4double z = z0;
    
    moveGermanium[i] = G4ThreeVector(DetectionSystemTigress::TransX(x,y,z,theta,phi),
				     DetectionSystemTigress::TransY(x,y,z,theta,phi),
				     DetectionSystemTigress::TransZ(x,z,theta));
	...
	if(j%2)
	  ...
	  ge_log->SetVisAttributes(yellow);
      }
      else {
	if(j%2)
	  ge_log->SetVisAttributes(yellow);
    ...
    if(!full) {
      ...
      new G4PVPlacement(rotateGermanium[i],moveGermanium[i],geDeadL,
			"GeDeadLayers",exp_hall_log,0,copyDL,fSurfCheck);
    }
  }
...
void DetectionSystemTigress::PlaceSuppressors(G4LogicalVolume* exp_hall_log, G4int detectorNumber) {
  ...
  G4VisAttributes* supVisAtt = new G4VisAttributes(G4Colour::Cyan());
  supVisAtt->SetVisibility(true);
  ...
  if(fIncludeBackSuppressors) {
    ...
    for(i=start; i<stop; i++) {
      ...
      G4int copy = 10*(detectorNumber*4 + i + 1) + 5; //5 is back suppressor label
      G4RotationMatrix* rotateBackPVP = new G4RotationMatrix(rotateBackQuarterSuppressor[i]->inverse());
      new G4PVPlacement(rotateBackPVP,moveBackQuarterSuppressor[i],fBackQuarterSuppressorLog,"BackSuppressor",
      		exp_hall_log,0,copy,fSurfCheck);
    }
  }
  ...
  if(fIncludeSideSuppressors) {
    ...
    for(i=start; i<stop; i++) {
      ...
      G4int copyR = 10*(detectorNumber*4 + i + 1) + 3; //3 is right side suppressor label
      G4RotationMatrix* rotateRSidePVP = new G4RotationMatrix(rotateSideSuppressor[2*i]->inverse());
      new G4PVPlacement(rotateRSidePVP,moveInnerSuppressor[2*i],fRightSuppressorLog,"RightSideSuppressor",
			exp_hall_log,0,copyR,fSurfCheck);
      ...
      G4int copyL = 10*(detectorNumber*4 + i + 2) + 4; //4 is left side suppressor label
      if(i==3)
	copyL = 10*(detectorNumber*4 + 1) + 4;
      
      G4RotationMatrix* rotateLSidePVP = new G4RotationMatrix(rotateSideSuppressor[2*i+1]->inverse());
      new G4PVPlacement(rotateLSidePVP,moveInnerSuppressor[2*i+1],fLeftSuppressorLog,"LeftSideSuppressor",
			exp_hall_log,0,copyL,fSurfCheck);
    }
  }
  ...
  if(!(fSuppressorPositionSelector) && fIncludeExtensionSuppressors)
      ...
      for(i=start; i<stop; i++)
	{
	  ...
	  G4int copyR = 10*(detectorNumber*4 + i + 1) + 1; //1 is right front suppressor label
	  G4RotationMatrix* rotateRFrontPVP = new G4RotationMatrix(rotateExtension[2*i]->inverse());
	  new G4PVPlacement(rotateRFrontPVP,moveInnerExtension[2*i],fRightSuppressorExtensionLog,"RightFrontSuppressor",
	  		    exp_hall_log,0,copyR,fSurfCheck);
	  ...
	  G4int copyL = 10*(detectorNumber*4 + i + 2) + 2; //2 is left front suppressor label
	  if(i==3)
	    copyL = 10*(detectorNumber*4 + 1) + 2;
	  
	  G4RotationMatrix* rotateLFrontPVP = new G4RotationMatrix(rotateExtension[2*i+1]->inverse());
	  new G4PVPlacement(rotateLFrontPVP,moveInnerExtension[2*i+1],fLeftSuppressorExtensionLog,"LeftFrontSuppressor",
	  		    exp_hall_log,0,copyL,fSurfCheck);
	}
...
///////////////////////////////////////////////////////////////////////
void DetectionSystemTigress::BuildelectrodeMatElectrodes() {
...
///////////////////////////////////////////////////////////////////////
void DetectionSystemTigress::ConstructDetector()  {
...
///////////////////////////////////////////////////////////////////////
G4SubtractionSolid* DetectionSystemTigress::FullQuarterDetector() {
  ...
  G4double holeHalfLengthZ = fGermaniumLength/2.0;
  ...
  return choppedGermanium3;

}//end ::FullQuarterDetector

G4SubtractionSolid* DetectionSystemTigress::QuarterDetector(G4double frontLayer, G4double backLayer, G4double innerLayer, G4double outterLayer) {
  
  G4double halfWidthX     = fGermaniumWidth/2.0 - outterLayer;
  G4double halfWidthY     = halfWidthX;
  G4double halfLengthZ    = fGermaniumLength/2.0;
  ...
  G4double innerRadius = fGermaniumOuterRadius - outterLayer;
  G4double startAngle = 0.0*M_PI;
  G4double finalAngle = 2.0*M_PI;
  ...
  G4double baseInnerRadius = fGermaniumOuterRadius - outterLayer;
  G4double baseOuterRadius = 2.0*baseInnerRadius;
  ...
  G4double initialAngle = acos((fGermaniumWidth/2.0 + fGermaniumShift - outterLayer)
			       /(fGermaniumOuterRadius - outterLayer));
  G4double totalAngle = M_PI/2.0 -2.0*initialAngle;
  ...
  // now we make the diagonal slices
  G4double halfChopPieceWidthX    = fGermaniumWidth/2.0 + outterLayer;
  G4double halfChopPieceWidthY    = fGermaniumWidth/2.0 + outterLayer;
  G4double halfChopPieceLengthZ   = (fGermaniumBentLength/cos(fBentEndAngle))/2.0 + outterLayer;
  G4Box* chopPiece = new G4Box("chopPiece", halfChopPieceWidthX,
			       halfChopPieceWidthY, halfChopPieceLengthZ);
  ...
  G4ThreeVector moveChopPiece1(0, fGermaniumWidth/2.0 +(fGermaniumBentLength
							/tan(fBentEndAngle) +fGermaniumWidth
							*cos(fBentEndAngle))/2.0 -((fGermaniumBentLength
										    /cos(fBentEndAngle))/2.0) /sin(fBentEndAngle) - outterLayer,
			       fGermaniumLength/2.0 -fGermaniumBentLength
			       +(fGermaniumBentLength +fGermaniumWidth
				 *sin(fBentEndAngle))/2.0 - outterLayer);
  ...
  G4ThreeVector moveChopPiece2(-(fGermaniumWidth/2.0 +(fGermaniumBentLength
						       /tan(fBentEndAngle) +fGermaniumWidth
						       *cos(fBentEndAngle))/2.0 -((fGermaniumBentLength
										   /cos(fBentEndAngle))/2.0) /sin(fBentEndAngle)) + outterLayer, 0,
			       fGermaniumLength/2.0 -fGermaniumBentLength
			       +(fGermaniumBentLength +fGermaniumWidth
				 *sin(fBentEndAngle))/2.0 - outterLayer);
  ...
  startAngle = 0.0*M_PI;
  finalAngle = 2.0*M_PI;
  G4double holeRadius = fGermaniumHoleRadius + innerLayer;
  //G4double holeHalfLengthZ = (fGermaniumLength - fGermaniumHoleDistFromFace)/2.0;
  G4double holeHalfLengthZ = fGermaniumLength/2.0;
			      
  G4Tubs* holeTubs = new G4Tubs("holeTubs",0.0,holeRadius,holeHalfLengthZ,startAngle,finalAngle);
  G4ThreeVector moveHole(fGermaniumShift,-fGermaniumShift,innerLayer-fGermaniumHoleDistFromFace);

  G4SubtractionSolid* choppedGermanium3 = new G4SubtractionSolid("choppedGermanium3",
								 choppedGermanium2, holeTubs, 0, moveHole);

  //(Fake) dead layer on front face
  G4Box* frontDLS = new G4Box("frontDLS",1.1*halfWidthX,1.1*halfWidthY,frontLayer);
  G4ThreeVector moveFrontDL(fGermaniumShift,-fGermaniumShift,halfLengthZ);

  G4SubtractionSolid* choppedGermanium4 = new G4SubtractionSolid("choppedGermanium4",
								 choppedGermanium3,frontDLS,0,moveFrontDL);

  //(Fake) dead layer on back
  G4Box* backDLS = new G4Box("backDLS",1.1*halfWidthX,1.1*halfWidthY,backLayer);
  G4ThreeVector moveBackDL(fGermaniumShift,-fGermaniumShift,-halfLengthZ);

  G4SubtractionSolid* choppedGermanium5 = new G4SubtractionSolid("choppedGermanium5",
								 choppedGermanium4,backDLS,0,moveBackDL);
  
  return choppedGermanium5;

}//end ::QuarterDetector2

std::vector<G4IntersectionSolid*> DetectionSystemTigress::SegmentedQuarterDetector(G4double frontLayer, G4double backLayer, G4double innerLayer,
										   G4double outterLayer, G4double middleLayer, G4bool full) {
  
  G4SubtractionSolid* ge_total = NULL;
  if(!full)
    ge_total = QuarterDetector(frontLayer,backLayer,innerLayer,outterLayer);
  else {
    ge_total = FullQuarterDetector();
    middleLayer = 0.0*mm;
  }
  ...
  G4Box* box = new G4Box("box",halfWidthX,halfWidthY,halfLengthZ);
  std::vector<G4IntersectionSolid*> segments;
  for(G4int i=0;i<8;i++) {

    G4ThreeVector shift(halfWidthX,halfWidthY,2.*halfLengthZ - 31.0*mm + middleLayer);
    if(i>3)
      shift.setZ(-31.0*mm - middleLayer);
      
    if(i==1 || i==5)
      shift.setY(-1.0*shift.getY());
