...
Excitation_Messenger::Excitation_Messenger(Excitation* exc, G4bool prj) : excitation(exc), proj(prj) {
  ...
  pLS_cmd->AvailableForStates(G4ApplicationState::G4State_PreInit,G4ApplicationState::G4State_Idle);
  ...
  pPF_cmd->AvailableForStates(G4ApplicationState::G4State_PreInit,G4ApplicationState::G4State_Idle);
  ...
  pSel_cmd->AvailableForStates(G4ApplicationState::G4State_PreInit,G4ApplicationState::G4State_Idle);
  ...
  pCon_cmd->AvailableForStates(G4ApplicationState::G4State_PreInit,G4ApplicationState::G4State_Idle);
  ...
  pGSS_cmd->AvailableForStates(G4ApplicationState::G4State_PreInit,G4ApplicationState::G4State_Idle);
  ...
  sCon_cmd->AvailableForStates(G4ApplicationState::G4State_PreInit,G4ApplicationState::G4State_Idle);
...
void Excitation_Messenger::SetNewValue(G4UIcommand* command, G4String newValue) {
  ...
  if(command == pLS_cmd) {
    ...
    message = "Setting " + nuc + " level scheme file to " + newValue;
  }

  else if(command == pPF_cmd) {
    excitation->SetPrbFile(newValue);
    message = "Setting " + nuc + " excitation probabilites file to " + newValue;
  }

  else if(command == pSel_cmd) {
    excitation->FixState(pSel_cmd->GetNewIntValue(newValue));
    message = "Selecting state " + newValue + " to populate in the " + nuc;
  }

  else if(command == pCon_cmd) {
    excitation->OnlyConsiderState(pCon_cmd->GetNewIntValue(newValue));
    message = "Will only consider state " + newValue + " when populating excited states in the "
	   + nuc;
  }

  else if(command == pGSS_cmd) {
    excitation->SetGSS(pGSS_cmd->GetNewDoubleValue(newValue));
    message = "Setting ground state spin of the " + nuc + " to " + newValue;
  }
